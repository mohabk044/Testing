...
def signup(request):
    if request.method == "POST" and "btnsignup" in request.POST:
        ...
        # fname = request.POST['fname']   # not to be done       #call with name
    ...
    if request.method == "POST" and "btnsave" in request.POST:
        if request.user is not None and request.user.id != None:
            ...
            if (
                ...
                and request.POST["address2"]
                and request.POST["city"]
                and request.POST["state"]
                and request.POST["zip"]
...
def product_favorite(request, pro_id):
    if request.user.is_authenticated and not request.user.is_anonymous:
        ...
        if UserProfile.objects.filter(
            user=request.user, product_favorites=pro_fav
        ).exists():
